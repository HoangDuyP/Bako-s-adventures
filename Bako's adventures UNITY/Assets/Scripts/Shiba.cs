...
public class Shiba : MonoBehaviour
{
    ...
    {
        float horizontalInput = Input.GetAxis("Horizontal Left");
        float verticalInput = Input.GetAxis("Vertical Left");
    ...
    private void Jump()
    {
        ...
        if (Input.GetButtonDown("Jump Left") && Ground_Check())
        {
    ...
    private bool Ground_Check()
    {
        ...
        if (Physics.Raycast(Shiba_box.bounds.center, RayDirection, out hit, Shiba_box.bounds.extents.y))
        {
            ...
            {
                RespawnPoint_Shiba.position = transform.position;
                return true; 
            }
